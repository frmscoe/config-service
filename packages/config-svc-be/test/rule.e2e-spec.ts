...
describe('AppController (e2e)', () => {
  ...
  it('/rule-and-its-configs/:name (GET) should return rules with their configurations', async () => {
    const name = (await createRule()).name;
    const response = await request(app.getHttpServer())
      .get(`/rule/rule-and-its-configs/${name}`)
      .set('Authorization', `Bearer ${userToken}`);
    expect(response.status).toBe(200);
  });
});

  ...
  const columns: TableColumnsType<ITypology> = useMemo(() => {
    return [
      ...
      },
      {
        ...
        filters: uniqueArray(data, "state").map((obj) => ({
          text: obj.state,
          value: obj?.state?.toLowerCase(),
        })),
      },
      {
        ...
        filters: uniqueArray(data, "ownerId").map((obj) => ({
          text: obj.ownerId,
          value: obj.ownerId.toLowerCase(),
        })),
      },
      {
      ...
      },
      {
        ...
        render: (_, record) => (
          <Space size="middle">
            {canEditTypology && (
              <Button type="link">{commonTranslations("typologyListPage.table.modify")} </Button>
            )}
            {canReviewTypology && (
              <Link href={`/typology/${record._key}/score`} type="link" className="text-blue-500">
                Score{" "}
              </Link>
            )}
          </Space>

...
export const Rules: React.FunctionComponent<RuleProps> = ({
    ...
    useEffect(() => {
      if (ruleOptions.length && allRules.length === 0) {
        setAllRules(ruleOptions); // use full rules list received from parent
      }
    }, [ruleOptions]);
    ...
    const handleSearch = (val: any) => {
      if (val.trim().length) {
        ...
        setRuleOptions(allRules); // Restore original rule list
      }
    ...
    return <div className="mx-0 px-0 w-full h-full" data-testid="rules-content">
...

        {
            (sortAlphabetically(ruleOptions, 'name')).map((r: any, index) => {
                return <div
                    key={index}
                    onClick={() => handleSelectRule(r._key)}

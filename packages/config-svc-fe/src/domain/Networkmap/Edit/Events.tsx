...

            {
                (sortAlphabetically(eventOptions, 'label')).map((r: IEvent, index) => {
                    ...
                    return <div
                        ...
                        style={{
                            border: selectEvent === r.value ? '2px solid #4CAE47' : '',
                            cursor: 'grab',
                        }}
                        className="flex justify-between items-center border-gray-400 px-2 border mx-1 p-2 mb-2"
                    >
                        <FileDoneOutlined className="w-1/4" style={{ fontSize: '1rem' }} />

...
export const getTypology = (id: string) => {
  return Api.get(`/typology/${encodeURIComponent(id)}`);
}
...
export const getNetworkMap = (id: string) => {
 return Api.get(`/network-map/${encodeURIComponent(id)}`);
}

export const updateNetworkMap = (id: string, payload: any) => {
  return Api.patch(`/network-map/${encodeURIComponent(id)}`, payload);
}

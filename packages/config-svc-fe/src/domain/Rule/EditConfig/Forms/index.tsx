...
export const ConfigForm: React.FunctionComponent<FormProps> = ({
    ...
    const { control, handleSubmit, formState, watch, setValue, reset, getValues, setError } = useForm({
      ...
      defaultValues: {
        ...
        isBand: props.configToEdit?.config?.bands?.length > 0 || false,
        isCase: props.configToEdit?.config?.cases?.length > 0 || false,
        category: props.configToEdit?.config?.bands?.length > 0 ? 'band' : props.configToEdit?.config?.cases?.length > 0 ? 'case' : '',
        dataType: props.rule?.dataType || 'NUMERIC',
        description: props.configToEdit?.desc || '',
        exitConditions: props.configToEdit?.config?.exitConditions || [],
        bands: props.configToEdit?.config?.bands || [],
        cases: props.configToEdit?.config?.cases || [],
        parameters: props.configToEdit?.config?.parameters || [],
      },
    });
    ...
    useEffect(() => {
      if (props.configToEdit) {
        const config = props.configToEdit.config || {};
        const [major, minor, patch] = props.configToEdit.cfg?.split('.').map(Number);

        reset({
          major: major || 0,
          minor: minor || 0,
          patch: patch || 0,
          isBand: config.bands?.length > 0,
          isCase: config.cases?.length > 0,
          dataType: props.rule?.dataType || 'NUMERIC',
          description: props.configToEdit.desc,
          category: config.bands?.length ? 'band' : config.cases?.length ? 'case' : '',

          parameters: config.parameters || [],

          exitConditions: (config.exitConditions || []).map((cond: any) => ({
            id: cond.subRuleRef,
            reason: cond.reason
          })),

          bands: (config.bands || []).map((band: any) => ({
            reason: band.reason,
            value: band.upperLimit, // or whatever field stores the correct value
            dataType: props.rule?.dataType
          })),

          cases: (config.cases || []).map((cs: any) => ({
            subRuleRef: cs.subRuleRef,
            reason: cs.reason,
            value: cs.value,
            dataType: props.rule?.dataType
          })),

          bandMinimumCondition: config.bands?.[0]?.lowerLimit ?? -99999999999,
          bandMaximumCondition: config.bands?.[config.bands.length - 1]?.lowerLimit ?? 99999999999,
          bandMaxReason: config.bands?.[config.bands.length - 1]?.reason ?? ''
        });

        if (config.bands?.length) setSelectedCategory('band');
        else if (config.cases?.length) setSelectedCategory('case');
      }
    }, [props.configToEdit, props.rule, reset]);

...
for PACKAGE in ${CHANGED_PACKAGES}; do
    ...
    PACKAGE_CUR_VERSION=$(cat ${PACKAGE_PATH}/package.json | jq -r ".version")
    PACKAGE_PUBLISH_FLAG=$(cat $PACKAGE_PATH/package.json | jq -r ".lextego.publish_to_dockerhub // false")
    ...
    if [[ ! "$PACKAGE_PUBLISH_FLAG" = "true" ]]; then
        echo -e "\n\tIGNORING package without 'lextego.publish_to_dockerhub' flag set to 'true' in package.json"
        continue
    fi

...
async function getPipelines() {
  ...
  if (pipelinesNextPageToken) url += `?page-token=${pipelinesNextPageToken}`;
...
async function getPipelineWorkflows(pipelineId) {
  ...
  if (workflowsResp.items.length <= 0) return null;
...
async function startLoop() {
  while (true) {
    ...
    for (const pipeline of pipelineList) {
      ...
      if (debug)
        console.log(
          `Pipeline with num: ${pipeline.number} state: ${pipeline.state} created at: ${pipeline.created_at} vcs_revision: ${pipeline.vcs.revision} - workflow count: ${workflowList.length} by user: ${pipeline.trigger.actor.login}`,
        );

      let anyFailedWorkflow = false;
      for (const workflow of workflowList) {
        if (debug) console.log(`\tWorkflow name: ${workflow.name} status: ${workflow.status}`);
